...
np.random.shuffle(data) #shuffle the loaded data
...
for train, test in kfold.split(X, Y):  #loop through the kfold split
    ...
    if history.history['val_acc'][-1] > highest_accuracy:
        ...
        with open(csv_file, 'w') as f: #write the results in a csv file
            for key in history.history.keys():
                f.write("%s,%s\n"%(key,history.history[key]))

        #save the model
        if os.path.isfile(r"genderclassification.h5") is False: #save the model
            model.save(r"genderclassification.h5")
        else:
            os.remove(r"genderclassification.h5")
            model.save(r"genderclassification.h5")
